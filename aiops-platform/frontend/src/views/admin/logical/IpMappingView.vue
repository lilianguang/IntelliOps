<template>
  <div>
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:15px">
      <h2 style="margin:0">地址映射</h2>
      <div>
        <el-button :icon="Refresh" @click="syncData" :loading="syncing">同步NAT</el-button>
    ...
    <el-card shadow="never" style="margin-bottom:15px">
      <el-form :inline="true" @submit.prevent="load">
        <el-form-item>
          <el-input v-model="keyword" placeholder="搜索IP/业务..." clearable style="width:280px" :prefix-icon="Search" @keyup.enter="load" />
        </el-form-item>
    ...
    <!-- 新增/编辑弹窗 -->
    <el-dialog v-model="showForm" :title="editId ? '编辑映射' : '新增映射'" width="680px" destroy-on-close>
      <el-form :model="form" label-width="100px">
        <el-row :gutter="16">
          <el-col :span="12"><el-form-item label="外网IP"><el-input v-model="form.external_ip" placeholder="如 211.147.153.1" /></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="外网端口"><el-input-number v-model="form.external_port" :min="1" :max="65535" controls-position="right" style="width:100%" /></el-form-item></el-col>
        </el-row>
        <el-row :gutter="16">
          <el-col :span="12"><el-form-item label="协议"><el-select v-model="form.protocol" clearable style="width:100%"><el-option label="tcp" value="tcp" /><el-option label="udp" value="udp" /><el-option label="icmp" value="icmp" /></el-select></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="VPN实例"><el-input v-model="form.vpn_instance" placeholder="VPN实例" /></el-form-item></el-col>
        </el-row>
        <el-row :gutter="16">
          <el-col :span="12"><el-form-item label="内网IP"><el-input v-model="form.internal_ip" placeholder="如 192.67.0.39" /></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="内网端口"><el-input-number v-model="form.internal_port" :min="1" :max="65535" controls-position="right" style="width:100%" /></el-form-item></el-col>
        </el-row>
        <el-row :gutter="16">
...
const loadMeta = async () => {
  try {
    const res = await logicalApi.getMeta('ip_mapping')
...
const load = async () => {
  ...
  try {
    ...
    const res = await logicalApi.ipMappingList(params)
...
const syncData = async () => {
  syncing.value = true
  try {
    const res = await logicalApi.ipMappingSync()
...
const handleExport = async () => {
  try {
    const res = await logicalApi.ipMappingExport()
    const url = URL.createObjectURL(res.data)
    const a = document.createElement('a')
    a.href = url; a.download = '地址映射.xlsx'; a.click()
...
const handleImport = async (file) => {
  ...
  try {
    const res = await logicalApi.ipMappingImport(formData)
...
const defaultForm = () => ({ external_ip: '', external_port: null, protocol: '', vpn_instance: '', internal_ip: '', internal_port: null, business: '', remarks: '' })
...
const saveRecord = async () => {
  ...
  try {
    ...
    if (editId.value) {
      await logicalApi.ipMappingUpdate(editId.value, data)
      ElMessage.success('更新成功')
    } else {
      await logicalApi.ipMappingCreate(data)
...
const delRecord = async (row) => {
  try {
    ...
    await logicalApi.ipMappingDel(row.id)
...
const tagType = (v) => {
  ...
  if (['permit', 'enable', 'online', 'up', 'tcp'].includes(s)) return 'success'
  if (['deny', 'disable', 'offline', 'down', 'udp'].includes(s)) return 'warning'
  return 'info'
}
...
const saveColSetting = async () => {
  try {
    for (let i = 0; i < colSettingList.value.length; i++) {
      ...
      await logicalApi.updateColumn('ip_mapping', c.id, { visible: c.visible, width: c.width, sort_order: i })
    }
    await loadMeta()
...
const addCustomCol = async () => {
  ...
  try {
    await logicalApi.createColumn('ip_mapping', { ...newCol.value })
...
const deleteColMeta = async (col) => {
  try {
    ...
    await logicalApi.deleteColumn('ip_mapping', col.id)
